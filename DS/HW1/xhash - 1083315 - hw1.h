...
//    return traitsObj( keyVal ) % maxidx;
   ...
   {
       auto it1 = myVec.myData.myFirst[n * 2];
       size_t R = 0;
       if (it1 == myList.end() && myVec.myData.myFirst[n * 2 + 1] == myList.end()) return R;
       while (1)
       {
           ++R;
           if (it1 == myVec.myData.myFirst[n * 2 + 1]) break;
           ++it1;
       }
   ...
   void insert( const value_type &val )
   {
       ...
       if (myList.size() == maxidx)
       {
           ...
           for (auto it = copy.begin(); it != copy.end(); ++it) putIn(*it);
       }
       putIn(val);
   }
   ...
   void erase( const key_type &keyVal )
   {
       ...
       auto it2 = &myVec.myData.myFirst[bucket(keyVal) * 2];
       if (*it2 == *(it2 + 1)) *it2 = myList.end(), * (it2 + 1) = myList.end();
       else if (*it2 == it || *(it2 + 1) == it) *it2 == it ? ++ * it2 : -- * (it2 + 1);
   ...
   iterator find(const key_type& keyVal)
   {
       ...
       while (it != myList.end() && *it != keyVal)++it;
       if (it == myList.end()) return myList.end();
       else return *it == keyVal ? it : myList.end();
   }
...
/*value_type *str = new value_type[cells]();
      ...
      if (it1 == myList.end() && myVec.myData.myFirst[(n * 2) + 1] == myList.end())
          return R;

      while (1)
      {
          ++R;
          if (it1 == myVec.myData.myFirst[(n * 2) + 1])
              break;
          ++it1;
      }
      ...
      if (myList.size() == maxidx)
      {
          ...
          myList.clear();
          myVec.assignGrow(maxidx * 2, myList.end());
          for (auto it1 = copy.begin(); it1 != copy.end(); ++it1) putIn(*it1);
      }

      putIn(val);*/

      /* auto it1 = find(keyVal);
        if (it1 == myList.end()) return;
        auto it2 = &myVec.myData.myFirst[bucket(keyVal) * 2];
        if (*it2 == *(it2 + 1)) *it2 = myList.end(), * (it2 + 1) = myList.end();
        else if (*it2 == it1 || *(it2 + 1) == it1)
        {
            if (*it2 == it1) ++* it2;
            else --*(it2 + 1);
        }
        ...
        /*  int hash = bucket(keyVal);
          auto it1 = myVec.myData.myFirst[hash * 2];
          while (it1 != myVec.myData.myFirst[hash * 2 + 1] && *it1 != keyVal) ++it1;
